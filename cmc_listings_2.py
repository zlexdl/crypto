...
class CmcCoinInfo(Base):
    __tablename__ = "cmc_coin_info_2"  # 数据库中保存的表名字
        ...
        for data in datas:
            ...
            if max_supply == 0:
                if circulating_supply == 0:
                    _market_cap = price_USD * total_supply
                else:
                    _market_cap = price_USD * circulating_supply
            else:
                _market_cap = price_USD * max_supply
            ...
            if session.query(CmcCoinInfo).filter(CmcCoinInfo.symbol == symbol).count() == 0:
                ...
                content = '<table><thead><tr><th></th><th></th></tr></thead><tbody><tr><td>name：</td><td>{}</td></tr><tr><td>symbol：</td><td>{}</td></tr><tr><td>最大供应量：</td><td>{}</td></tr><tr><td>总供应量：</td><td>{}</td></tr><tr><td>流通供应量：</td><td>{}</td></tr><tr><td>价格(U)：</td><td>{}</td></tr><tr><td>平台：</td><td>{}</td></tr><tr><td>加入时间：</td><td>{}</td></tr><tr><td>地址：</td><td></td></tr><tr><td colspan="2">{}</td></tr></tbody></table>'.format(
                    name, symbol, max_supply, total_supply, circulating_supply, price, platform_name, date_added,
                    token_address)
            else:
                cmcCoinInfo = session.query(CmcCoinInfo).filter(CmcCoinInfo.symbol == symbol)
                cmcCoinInfo.update({
                    "symbol": symbol
                    , "name": name
                    , "slug": slug
